...
update_status ModuleHUD::Update() {
	...
	if (!App->player->maxPow) {
		if (App->player->level == 1) {
			sprintf_s(powText, 4, "%3d", (4 - App->player->total));
		}
		else if (App->player->level == 2) {
			sprintf_s(powText, 4, "%3d", (15 - App->player->total));
		}
		else if (App->player->level == 3) {
			sprintf_s(powText, 4, "%3d", (26 - App->player->total));
		}
		else if (App->player->level == 4) {
			sprintf_s(powText, 4, "%3d", (42 - App->player->total));
		}
		else if (App->player->level == 5) {
			sprintf_s(powText, 4, "%3d", (42 - App->player->total));
		}
	}
